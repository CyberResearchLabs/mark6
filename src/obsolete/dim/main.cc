...
run_tests() {
  ...
  std::list<boost::thread*> _receiver_threads;

  try {
    BOOST_FOREACH(Receiver r, c._receivers) {
      _receiver_threads.push_back(new boost::thread(ReceiverThread(),
						    r._ip,
						    r._port,
						    c._stream_rate,
						    c._duration,
  ...
  BOOST_FOREACH(boost::thread* t, _receiver_threads) {
    t->join();
  }
}
...
main (int argc, char* argv[])
{
  ...
  desc.add_options()
    ...
    (
     ...
     ->default_value(string("dim6-log.cfg")),
  ...
  // Start processing.
  try {
    LOG4CXX_INFO(logger, "Creating dim6 manager.");
