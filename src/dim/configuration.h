...
struct Receiver {
  ...
  // Ostream serializer.
  friend ostream& operator<<(ostream& out, Receiver r) {
    out
      << "Receiver {\n"
      << "  ip:  " << r._ip << "\n"
      << "  port:" << r._port << "\n"
...
typedef std::vector<Receiver> ReceiverVector;
...
struct Configuration {
  ...
  void load(const std::string &filename) {
    ...
    BOOST_FOREACH(ptree::value_type &v,
      ...
      ptree &rt = v.second;
      Receiver r;
      r._ip = rt.get<std::string>("<xmlattr>.ip");
      r._port = rt.get<int>("<xmlattr>.port");
