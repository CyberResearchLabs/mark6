...
/*
 * $Id: replay_main.c 583 2010-08-03 21:12:57Z gbc $
 *
...
int main(int argc, char **argv)
{
    ...
    errs += bmr_parse_cache_cmd("load:default");
    ...
    bsp->verb = lsp->verb = psp->verb = verb;
    errs += replay_exec_cache(bsp, lsp, psp);
    ...
    if (errs) return(fprintf(stderr, "set_load_state() failed (%d)\n", errs));
    errs += set_push_state(psp);
    if (errs) return(fprintf(stderr, "set_push_state() failed (%d)\n", errs));

    /* go into service */
    errs = bmr_replay_dispatch();
    if (errs) fprintf(stderr, "bmr_replay_dispatch() failed (%d)\n", errs);
    return(errs);
}
